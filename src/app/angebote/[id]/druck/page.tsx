...
export default async function QuotePrintPage({
  ...
  const { data: items } = await supabaseAdmin
    ...
    .is("deleted_at", null)
    .order("position_no");
  const { data: customer } = quote
    ? await supabaseAdmin.from("customers").select("*").eq("id", quote.customer_id).single()
    : { data: null };
      ...
      <Card className="print:shadow-none">
        <CardContent className="space-y-8 p-8">
          <header className="flex items-start justify-between gap-6">
            <div>
              <p className="text-sm uppercase tracking-[0.24em] text-ink-500">Angebot</p>
              <h1 className="mt-3 font-display text-5xl text-ink-900">{quote.title}</h1>
              <p className="mt-3 text-sm text-ink-700">{quote.quote_number}</p>
            </div>
          ...
          <section className="grid gap-6 md:grid-cols-2">
            ...
            <div>
              <h2 className="text-sm font-semibold uppercase tracking-[0.2em] text-ink-500">Hinweise</h2>
              <p className="mt-2 text-sm text-ink-700">{quote.notes ?? "Keine zusaetzlichen Hinweise."}</p>
              <p className="mt-4 text-sm text-ink-700">
                Zahlungsbedingungen: {quote.payment_terms ?? "-"}
              </p>
            </div>
          </section>
